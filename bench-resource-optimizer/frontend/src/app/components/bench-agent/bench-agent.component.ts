...
@Component({
  ...
  template: `
    @if (auth.isLoggedIn()) {
      <button class="ai-fab" (click)="agent.toggle()" [class.open]="agent.isOpen()" aria-label="Ask Aria">
        <div class="fab-av-wrap"><img src="aarav-happy.svg" class="fab-av" alt="Aria"/><span class="fab-pulse"></span></div>
        <div class="fab-txt"><span class="fab-name">Aria</span><span class="fab-sub">HR AI · Ask me!</span></div>
      ...
      @if (agent.isOpen()) {
        <div class="ai-panel" [style.width.px]="pw()" [style.height.px]="ph()" role="dialog">
          ...
          <div class="ai-header">
            ...
            <div class="h-info"><strong>Aria</strong><span>Bench Optimizer · HR Guide ✦</span></div>
          ...
          <div class="ai-msgs" #scrollRef>
            @if (!agent.hasMessages()) {
              <div class="welcome">
                ...
                <p class="w-sub">Your HR AI guide for Bench Resource Optimizer. Ask me about your match score, skill gaps, or how the platform works.</p>
                <div class="q-grid">
            ...
            @if (agent.isLoading()) {
              <div class="msg ma"><img src="aarav-thinking.svg" class="m-av" alt="Aria"/><div class="bubble ba typing"><span></span><span></span><span></span></div></div>
            }
          ...
          <div class="ai-input">
            @if (agent.isLimited()) {
              ...
              <input class="inp" type="text" [placeholder]="'Ask about your results… (' + agent.qRemaining() + ' left)'" [(ngModel)]="draft" (keydown.enter)="send(draft)" [disabled]="agent.isLoading()||agent.isStreaming()" maxlength="500"/>
  ...
  `,
  styles: [`
    .ai-fab { position:fixed; bottom:24px; right:24px; z-index:10000; display:flex; align-items:center; gap:10px; padding:8px 16px 8px 8px; border-radius:50px; background:#fff; border:1.5px solid rgba(245,158,11,0.35); cursor:pointer; box-shadow:0 4px 16px rgba(245,158,11,0.2),0 1px 4px rgba(0,0,0,0.08); transition:transform .2s,box-shadow .2s; }
    .ai-fab:hover { transform:translateY(-2px); box-shadow:0 8px 24px rgba(245,158,11,0.3); }
    .ai-fab.open { background:#fffbeb; border-color:#f59e0b; }
    .fab-av-wrap { position:relative; width:40px; height:40px; flex-shrink:0; }
    .fab-av { width:40px; height:40px; border-radius:50%; object-fit:contain; background:#fffbeb; padding:2px; }
    .fab-pulse { position:absolute; bottom:0; right:0; width:11px; height:11px; border-radius:50%; background:#f59e0b; border:2px solid #fff; animation:pulse 2s infinite; }
    @keyframes pulse { 0%,100%{box-shadow:0 0 0 0 rgba(245,158,11,.5)} 50%{box-shadow:0 0 0 5px rgba(245,158,11,0)} }
    .fab-txt { display:flex; flex-direction:column; line-height:1.2; }
    .fab-name { font-size:13px; font-weight:800; color:#1a2540; }
    .fab-sub  { font-size:10px; color:#d97706; }
    ...
    .ai-panel { position:fixed; bottom:90px; right:24px; z-index:9999; display:flex; flex-direction:column; border-radius:18px; background:#fff; border:1px solid rgba(245,158,11,0.2); box-shadow:0 12px 40px rgba(245,158,11,0.12),0 2px 8px rgba(0,0,0,0.08); overflow:hidden; animation:su .22s ease; min-width:300px; min-height:340px; max-width:90vw; max-height:90vh; }
    @keyframes su { from{opacity:0;transform:translateY(14px) scale(.98)} to{opacity:1;transform:translateY(0) scale(1)} }
    .resize-h { position:absolute; top:6px; left:8px; font-size:14px; color:rgba(245,158,11,.4); cursor:nw-resize; user-select:none; z-index:10; transition:color .15s; }
    .resize-h:hover { color:rgba(245,158,11,.9); }
    .ai-header { display:flex; align-items:center; gap:10px; padding:12px 14px 12px 28px; background:linear-gradient(135deg,#fffbeb,#fef3c7); border-bottom:1px solid rgba(245,158,11,.15); flex-shrink:0; }
    .h-av { width:36px; height:36px; border-radius:50%; background:#fffbeb; flex-shrink:0; border:2px solid rgba(245,158,11,.2); }
    .h-info { flex:1; }
    .h-info strong { display:block; color:#1a2540; font-size:13px; font-weight:800; }
    .h-info span   { color:#d97706; font-size:10px; }
    .h-btn { background:none; border:none; color:#d97706; font-size:14px; cursor:pointer; padding:5px 7px; border-radius:7px; transition:background .15s; line-height:1; display:flex; align-items:center; }
    .h-btn:hover { background:rgba(245,158,11,.1); }
    .h-close { font-size:16px; }
    .ai-msgs { flex:1; overflow-y:auto; padding:14px 12px; display:flex; flex-direction:column; gap:12px; background:#fffdf5; scrollbar-width:thin; scrollbar-color:rgba(245,158,11,.25) transparent; }
    .welcome { text-align:center; padding:16px 8px 8px; }
    .w-av { width:80px; height:80px; margin:0 auto 12px; display:block; border-radius:50%; background:#fffbeb; border:3px solid rgba(245,158,11,.2); padding:4px; object-fit:contain; }
    ...
    .q-btn { padding:8px 10px; border-radius:10px; background:#fff; border:1px solid rgba(245,158,11,.25); color:#92400e; font-size:11px; cursor:pointer; text-align:left; transition:background .15s; line-height:1.35; font-weight:500; }
    .q-btn:hover { background:#fffbeb; border-color:#f59e0b; }
    .msg { display:flex; align-items:flex-end; gap:7px; }
    .mu { flex-direction:row-reverse; } .ma { flex-direction:row; }
    .m-av { width:30px; height:30px; border-radius:50%; flex-shrink:0; margin-bottom:16px; background:#fffbeb; border:2px solid rgba(245,158,11,.15); padding:2px; object-fit:contain; }
    ...
    .bu { background:linear-gradient(135deg,#f59e0b,#d97706); color:#fff; border-bottom-right-radius:4px; }
    .ba { background:#fff; color:#1a2540; border-bottom-left-radius:4px; border:1px solid rgba(245,158,11,.12); box-shadow:0 1px 4px rgba(0,0,0,.06); }
    .ba h2,.ba h3,.ba h4 { color:#92400e; font-weight:700; margin:6px 0 3px; }
    .ba strong { color:#1a2540; font-weight:700; } .ba em { color:#d97706; }
    .ba code { background:#fffbeb; color:#92400e; padding:1px 5px; border-radius:4px; font-size:11.5px; }
    .ba ul { margin:4px 0 4px 16px; } .ba li { margin:3px 0; color:#374151; }
    .ba hr { border:none; border-top:1px solid rgba(245,158,11,.15); margin:8px 0; }
    .mt { font-size:10px; color:#94a3b8; margin-top:3px; }
    .cur { animation:blink .7s step-end infinite; color:#f59e0b; }
    @keyframes blink { 50%{opacity:0} }
    .typing { display:flex !important; gap:5px; align-items:center; padding:12px 16px !important; }
    .typing span { width:7px; height:7px; border-radius:50%; background:#f59e0b; animation:bounce 1.2s infinite; }
    ...
    .ai-input { display:flex; gap:7px; padding:10px 12px; border-top:1px solid rgba(245,158,11,.12); background:#fff; flex-shrink:0; }
    .inp { flex:1; padding:9px 13px; border-radius:10px; background:#fffdf5; border:1px solid rgba(245,158,11,.25); color:#1a2540; font-size:13px; outline:none; transition:border-color .2s; font-family:inherit; }
    .inp::placeholder { color:#94a3b8; } .inp:focus { border-color:#f59e0b; background:#fff; } .inp:disabled { opacity:.45; }
    .send { width:38px; height:38px; flex-shrink:0; border-radius:10px; background:linear-gradient(135deg,#f59e0b,#d97706); border:none; color:#fff; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:opacity .15s,transform .15s; }
...
export class BenchAgentComponent implements AfterViewChecked, OnDestroy {
  agent=inject(BenchAgentService); auth=inject(AuthService); sanitizer=inject(DomSanitizer);
