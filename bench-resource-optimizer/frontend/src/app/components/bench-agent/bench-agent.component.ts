...
@Component({
  ...
  template: `
    @if (auth.isLoggedIn()) {
      ...
      @if (agent.isOpen()) {
        <div class="b-panel" [style.width.px]="pw()" [style.height.px]="ph()" role="dialog">
          ...
          <div class="b-msgs" #scrollRef>
            ...
            @if (agent.error()) {
              <div class="err-bar">⚠️ {{ agent.error() }}<button (click)="agent.error.set(null)">✕</button></div>
            }
          </div>
          ...
          <div class="b-input">
            @if (agent.isLimited()) {
              ...
              <input class="inp" type="text"
                [placeholder]="'Ask about HR, skills, roles… (' + agent.qRemaining() + ' left)'"
                [(ngModel)]="draft"
  ...
  `,
  styles: [`
    /* FAB — blue, matches Bench primary #3b82f6 */
    .b-fab {
      ...
      cursor: pointer;
      box-shadow: 0 4px 16px rgba(59,130,246,0.2), 0 1px 4px rgba(0,0,0,0.08);
      transition: transform .2s, box-shadow .2s;
    }
    .b-fab:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(59,130,246,0.3); }
    ...
    .fab-sub  { font-size: 10px; color: #3b82f6; }
    .fab-badge {
    ...
    .b-panel {
      ...
      box-shadow: 0 12px 40px rgba(59,130,246,0.15), 0 2px 8px rgba(0,0,0,0.08);
      overflow: hidden;
    ...
    .resize-h {
      ...
      font-size: 14px; color: rgba(59,130,246,.35);
      cursor: nw-resize; user-select: none; z-index: 10;
      transition: color .15s;
    }
    .resize-h:hover { color: rgba(59,130,246,.8); }
    ...
    .h-btn:hover { background: rgba(59,130,246,.1); color: #2563eb; }
    ...
    .w-av {
      width: 80px; height: 80px; margin: 0 auto 12px; display: block;
      border-radius: 50%; background: #eff6ff;
      border: 3px solid rgba(59,130,246,.2); padding: 4px; object-fit: contain;
    }
    .w-title { color: #1e293b; font-weight: 800; margin: 0 0 5px; font-size: 15px; }
    ...
    .q-btn {
      ...
      background: #ffffff; border: 1px solid rgba(59,130,246,.25);
    ...
    .bu { background: linear-gradient(135deg,#3b82f6,#2563eb); color: #fff; border-bottom-right-radius: 4px; }
    .ba {
    ...
    .ba code   { background: #eff6ff; color: #1d4ed8; padding: 1px 5px; border-radius: 4px; font-size: 11.5px; }
    .ba ul     { margin: 4px 0 4px 16px; } .ba li { margin: 3px 0; color: #374151; }
    .ba hr     { border: none; border-top: 1px solid rgba(59,130,246,.15); margin: 8px 0; }
    ...
    .typing span { width: 7px; height: 7px; border-radius: 50%; background: #3b82f6; animation: bounce 1.2s infinite; }
    ...
    .err-bar button { background: none; border: none; color: #dc2626; cursor: pointer; }
    ...
    .locked {
      ...
      padding: 8px 12px; background: #fef2f2; border-radius: 10px;
      border: 1px solid #fecaca; font-size: 12px; color: #dc2626; font-weight: 600;
    }
    ...
    .inp {
      ...
      background: #f8fafc; border: 1px solid rgba(59,130,246,.25);
    ...
    .send-btn:hover:not(:disabled) { opacity: .85; transform: scale(1.07); }
    .send-btn:disabled { opacity: .3; cursor: not-allowed; }

    @media (max-width: 480px) {
      .b-panel { width: calc(100vw - 20px) !important; right: 10px; bottom: 80px; }
      .b-fab   { right: 10px; bottom: 10px; }
...
export class BenchAgentComponent implements AfterViewChecked, OnDestroy {
  ...
  private _lmc = 0; private _rs = false;
  private _sx = 0; private _sy = 0; private _sw = 390; private _sh = 560;
  ...
  rsStart(e: MouseEvent) {
    ...
    this._om = (ev) => {
      if (!this._rs) return;
      this.pw.set(Math.max(300, Math.min(700, this._sw + (this._sx - ev.clientX))));
      this.ph.set(Math.max(340, Math.min(860, this._sh + (this._sy - ev.clientY))));
    };
    this._ou = () => {
      this._rs = false;
