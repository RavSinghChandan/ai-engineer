        ...
        return {
            ...
            "token_economics": {
                ...
                "avg_prompt_tokens": round(
                    sum(r.prompt_tokens for r in runs_with_tokens) / max(len(runs_with_tokens), 1)),
                "avg_completion_tokens": round(
                    sum(r.completion_tokens for r in runs_with_tokens) / max(len(runs_with_tokens), 1)),
                "avg_total_tokens": round(
                    sum(r.total_tokens for r in runs_with_tokens) / max(len(runs_with_tokens), 1)),
                "total_cost_usd": round(sum(r.cost_usd for r in records), 6),
                "avg_cost_per_request": round(
                    sum(r.cost_usd for r in runs_with_tokens) / max(len(runs_with_tokens), 1), 6),
