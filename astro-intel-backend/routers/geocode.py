...
class GeocodeSearchItem(BaseModel):
    display_name: str
    city:         str
    country:      str
    lat:          float
...
@router.get("/search", response_model=list[GeocodeSearchItem])
async def geocode_search(q: str, limit: int = 6) -> list[GeocodeSearchItem]:
    """
    Typeahead autocomplete: q (partial city name) → up to `limit` suggestions.
    Hits Nominatim with featuretype=city for clean results.
    Falls back to built-in table when Nominatim is unavailable or returns nothing.
    """
    ...
    # Search built-in table first for instant results
    query_lower = q.strip().lower()
    builtin_hits: list[GeocodeSearchItem] = []
    for key, val in _BUILTIN.items():
        if key.startswith(query_lower) or query_lower in key:
            parts = val["display_name"].split(",")
            builtin_hits.append(GeocodeSearchItem(
        ...
        if len(builtin_hits) >= limit:
            break

    # Try Nominatim for richer results
    global _LAST_CALL
    async with _NOM_LOCK:
        gap = time.time() - _LAST_CALL
        if gap < 1.0:
    ...
    if not results:
        return builtin_hits[:limit]

    items: list[GeocodeSearchItem] = []
    seen: set[str] = set()
    for r in results:
        lat = float(r["lat"])
        lon = float(r["lon"])
        ...
        city_name = (
            ...
            or addr.get("county") or r.get("display_name", q).split(",")[0]
        )
        country_name = addr.get("country", "")
        state_name   = addr.get("state", "")
        display = f"{city_name}, {state_name}, {country_name}" if state_name else f"{city_name}, {country_name}"
        key = display.lower()
        if key in seen:
            continue
        seen.add(key)
        items.append(GeocodeSearchItem(
    ...
    return items if items else builtin_hits[:limit]
