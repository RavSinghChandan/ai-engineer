...
export class NumerologyService {
  ...
  private _letterValueChaldean(c: string): number {
    // Chaldean system: 9 is sacred and NOT assigned to any letter.
    // Correct mapping differs from Indian Vedic — I/J/Y map to 1, not shared with Q.
    const map: Record<string, number> = {
      A:1,I:1,J:1,Y:1,        // 1
      B:2,K:2,R:2,             // 2
      C:3,G:3,L:3,S:3,         // 3
      ...
      U:6,V:6,W:6,             // 6
      O:7,Z:7,                 // 7
      F:8,P:8,Q:8,             // 8  (Q moves to 8 in Chaldean, not 1)
