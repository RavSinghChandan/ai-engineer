...
export class RunbookAgentService {

  private readonly agentUrl = 'http://localhost:8012/agent';
  ...
  constructor(private http: HttpClient) {
    this.sessionId = sessionStorage.getItem('runbookai_agent_session');
    const saved = sessionStorage.getItem('runbookai_agent_qcount');
  ...
  async chatStream(userMessage: string): Promise<void> {
    ...
    sessionStorage.setItem('runbookai_agent_qcount', String(newCount));
  ...
  private _handleLine(line: string, idx: number): void {
    ...
    try {
      ...
      if (ev.type === 'session') { this.sessionId = ev.session_id; sessionStorage.setItem('runbookai_agent_session', ev.session_id); }
  ...
  async clearSession(): Promise<void> {
    ...
    this.sessionId = null;
    sessionStorage.removeItem('runbookai_agent_session');
    sessionStorage.removeItem('runbookai_agent_qcount');
    this.qCount.set(0);
    this.messages.set([]);
    this.error.set(null);
  }
