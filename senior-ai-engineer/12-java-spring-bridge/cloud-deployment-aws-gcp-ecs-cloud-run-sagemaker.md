...
AstroIntel is a FastAPI service with 6 concurrent LLM calls per request and a full multi-tenant SaaS auth layer. No GPU needed (calling OpenAI/DeepSeek, not self-hosted). ECS Fargate is the right choice:
...
- Secrets in SSM Parameter Store, injected as ECS task secrets:
  - `MASTER_API_KEY` → bootstraps SUPERADMIN on first boot (no pre-configuration needed)
  - `JWT_SECRET` → signs and verifies all JWT Bearer tokens
  - `DEEPSEEK_API_KEY` / `OPENAI_API_KEY` → LLM provider keys
- Auth persistence: `auth_keys.json` mounted from EFS — survives container restarts, tenants and keys persist across deployments
- Monthly cost: 2 tasks × t3-equivalent × 730 hours ≈ $30-50/month for the compute

**AstroIntel auth on EC2 (single-server quick deploy):**

For the initial production deployment (1-2K users/day, personal/team use):
```
EC2 t3.medium → Docker container (uvicorn --workers 4)
               ├── MASTER_API_KEY in /etc/environment (non-root, not in source)
               ├── JWT_SECRET in /etc/environment
               ├── auth_keys.json on host volume (persists across container restarts)
               └── Nginx reverse proxy → port 8080 → HTTPS via Let's Encrypt

Roles in production:
  SUPERADMIN = you (MASTER_API_KEY, never shared)
  ADMIN = trusted team members (keys generated via POST /admin/tenants/{id}/keys)
  USER = customers (keys generated by ADMIN)
```
This is the next deployment phase — auth is implemented and tested (76/76 tests), the EC2 + Docker layer is next.
...
- How would you deploy AstroIntel on AWS?
  ...
  **Answer:** ECS Fargate with Application Load Balancer. Two Fargate services: `astro-api` (FastAPI + LangGraph pipeline, 2 vCPU / 4GB, 2 tasks minimum) and `astro-worker` (Celery workers for async analysis jobs, same sizing, auto-scales on queue depth). ALB in front of `astro-api` with target group health checks on `GET /health`. Redis via ElastiCache (SSE pub/sub, semantic cache). Postgres via RDS for session data and LangGraph checkpoints. OpenAI/DeepSeek API keys in SSM Parameter Store, injected as ECS task secrets. CloudWatch for logs and custom metrics (faithfulness score, token cost per run). S3 for audit log storage. Total infrastructure cost at demo scale: ~$50-80/month. Bench Resource Optimizer would follow the same ECS Fargate pattern: single FastAPI service (2 vCPU / 4GB) with FAISS loaded in-process from S3 on startup, Redis via ElastiCache for the L1/L2 semantic cache, and DeepSeek API key in SSM — stateless container design so scale-out adds capacity linearly.
