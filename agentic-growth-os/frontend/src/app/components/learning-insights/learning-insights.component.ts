  ...
  template: `
    <div class="space-y-5 animate-fade-in">
      <div>
        ...
        <p class="text-xs text-slate-500 mt-0.5">How the AI is improving campaigns over time</p>
      </div>
      ...
      <div class="glass-card">
        ...
        <div class="grid grid-cols-4 gap-3">
          <div *ngFor="let step of howItWorks" class="text-center p-3 rounded-xl bg-slate-50">
            <div class="text-2xl mb-2">{{ step.icon }}</div>
            <div class="text-xs font-semibold text-slate-900">{{ step.title }}</div>
            <div class="text-xs text-slate-500 mt-1">{{ step.desc }}</div>
          </div>
        </div>
      </div>
      ...
      <div *ngIf="!insights.length && totalLearned < 2" class="glass-card text-center py-12">
        ...
        <div class="text-slate-500 font-medium">Not enough data yet</div>
        <div class="text-sm text-slate-500 mt-1">Run the same campaign type at least twice to see learning in action</div>
      ...
      <div *ngFor="let ins of insights" class="glass-card animate-slide-up">
        <div class="flex items-start justify-between mb-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="font-bold text-slate-900 capitalize">{{ ins.campaign_type.replace('_', ' ') }}</div>
              <div class="text-xs text-slate-500">{{ ins.runs }} campaigns analyzed</div>
            </div>
          </div>
          <div class="text-right">
            <div class="text-2xl font-bold"
                 [class]="ins.roi_improvement > 0 ? 'text-emerald-600' : 'text-red-600'">
        ...
        <div class="space-y-3 mb-4">
          <div>
            <div class="flex justify-between text-xs mb-1">
              <span class="text-slate-500">CTR Improvement</span>
              <span [class]="ins.ctr_improvement >= 0 ? 'text-emerald-600' : 'text-red-600'">
          ...
          <div>
            <div class="flex justify-between text-xs mb-1">
              <span class="text-slate-500">Latest ROI Score</span>
              <span class="text-slate-900 font-semibold">{{ ins.latest_roi | number:'1.2-2' }}x</span>
            </div>
            <div class="h-1.5 rounded-full bg-slate-50 overflow-hidden">
        ...
        <div class="grid grid-cols-2 gap-2">
          <div class="p-3 rounded-xl bg-slate-50">
            ...
            <div class="text-sm font-semibold text-indigo-700 capitalize">{{ ins.best_tone.replace('_', ' ') }}</div>
          </div>
          <div class="p-3 rounded-xl bg-slate-50">
            <div class="label-text mb-1">Best Bid Strategy</div>
            <div class="text-sm font-semibold text-violet-700 capitalize">{{ ins.best_strategy.replace('_', ' ') }}</div>
          </div>
        </div>
      </div>
        ...
        <div class="space-y-2">
          <div *ngFor="let run of allRuns; let i = index"
            ...
            <div class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold"
                 [class]="run.learning_applied ? 'bg-emerald-500/20 text-emerald-600' : 'bg-indigo-500/20 text-indigo-600'">
              {{ i + 1 }}
            </div>
            <div class="flex-1">
              <div class="text-sm font-medium text-slate-900">{{ run.campaign_name }}</div>
            ...
            <div class="text-right">
              ...
              <div class="text-xs text-slate-500">CTR: {{ run.metrics.ctr | number:'1.1-2' }}%</div>
            </div>
      ...
      <div class="glass-card border-indigo-500/20 bg-indigo-500/5">
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="text-sm font-semibold text-indigo-700">This system improves automatically over time</div>
