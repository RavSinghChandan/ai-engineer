...
export class AgenticAgentService {
  ...
  readonly quickPrompts = [
    ...
    '📈 How does auto-learning improve ROI?',
    '🔁 What happens on a second campaign run?',
    '⚙️ What is LangGraph StateGraph?',
    '💡 How is the campaign memory used?',
  ...
  constructor(private http: HttpClient) {
    this.sessionId = sessionStorage.getItem('agentic_agent_session');
    const saved = sessionStorage.getItem('agentic_agent_qcount');
  ...
  async chatStream(userMessage: string): Promise<void> {
    ...
    sessionStorage.setItem('agentic_agent_qcount', String(newCount));
  ...
  private _handleLine(line: string, idx: number): void {
    ...
    try {
      ...
      if (ev.type === 'session') { this.sessionId = ev.session_id; sessionStorage.setItem('agentic_agent_session', ev.session_id); }
  ...
  async clearSession(): Promise<void> {
    ...
    this.sessionId = null;
    sessionStorage.removeItem('agentic_agent_session');
    sessionStorage.removeItem('agentic_agent_qcount');
    this.qCount.set(0);
    this.messages.set([]);
    this.error.set(null);
  }
