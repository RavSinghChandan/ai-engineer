...
def mine_hard_negatives(
    ...
    Args:
        ...
        output_format (Literal["triplet", "n-tuple", "labeled-pair", "labeled-list"]): Output format for the `datasets.Dataset`.
            ...
            - "labeled-pair": (anchor, document, label) tuples with a label of 0 for negative and 1 for positive, i.e. 3 columns. Useful for e.g. :class:`~sentence_transformers.cross_encoder.losses.BinaryCrossEntropyLoss`.
    ...
    if cache_folder:
        ...
        corpus_hash = hashlib.sha256(
            repr((model_name, corpus_prompt_name, corpus_prompt, tuple(corpus))).encode(),
            usedforsecurity=False,
        ).hexdigest()
